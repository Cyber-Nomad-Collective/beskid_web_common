...
function parseDomainTiles(
	body: string,
): { pathPrefix: string; heading: string } | null {
...
function main() {
	...
	for (const abs of files) {
		...
		if (cls === "legacy-or-bridge" || cls === "component" || cls === "article")
			continue;
		...
		const layout: LayoutContractFile = {
			...
			minMarkdownHeadings:
				cls === "domain-root"
					? minMarkdownRelax(presetKey, sig.markdownH2.length)
					: undefined,
		};
		...
		if (tiles) {
			layout.widgets = [
				{
					type: "domainTiles",
					props: { pathPrefix: tiles.pathPrefix, heading: tiles.heading },
				},
			];
		}
