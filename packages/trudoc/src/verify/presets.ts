...
const structureSteps = (roots: VerifyRoots): VerifyStep[] => [
	{
		...
		args: [script(roots.pkgRoot, "verify-platform-spec-frontmatter.mjs")],
	},
	{ cmd: process.execPath, args: [roots.tsxCli, roots.layoutVerifyTs] },
	{
		cmd: process.execPath,
		args: [script(roots.pkgRoot, "verify-graph-frontmatter.mjs")],
	},
	{
		cmd: process.execPath,
		args: [script(roots.pkgRoot, "verify-language-meta-related-links.mjs")],
	},
];
...
const contentStep = (roots: VerifyRoots, warnOnly: boolean): VerifyStep => ({
	cmd: process.execPath,
	args: [
		script(roots.pkgRoot, "verify-platform-spec-content.mjs"),
		...(warnOnly ? ["--warn-only"] : []),
	],
});
