...
const ARTICLE_ROLE_THRESHOLDS: Record<
	string,
	{ minH2: number; minBodyLines: number }
> = {
...
function loadFrontmatter(filePath: string): Record<string, unknown> {
	...
	if (end === -1) return {};
	return (
		(parseYaml(raw.slice(3, end).trim()) as Record<string, unknown> | null) ?? {}
	);
}
...
function proseMarksNonNormativeNearLink(
	body: string,
	legacyPrefix: "/execution/" | "/corelib/",
): boolean {
...
function extractMarkdownTableSignatures(body: string): string[] {
	...
	while (i < lines.length) {
		...
		signatures.push(
			block.map((row) => row.replace(/\s+/g, " ").toLowerCase()).join("\n"),
		);
	}
	return signatures;
}
...
function checkCircularCanon(
	rel: string,
	body: string,
): PlatformSpecContentIssue | null {
	if (
		LANGUAGE_META_CIRCULAR_CANON_ALLOWLIST.includes(
			rel as (typeof LANGUAGE_META_CIRCULAR_CANON_ALLOWLIST)[number],
		)
	) {
		return null;
	}
...
function checkHtmlEntityGenerics(
	rel: string,
	body: string,
): PlatformSpecContentIssue | null {
...
function checkStubExplainsBoilerplate(
	rel: string,
	body: string,
): PlatformSpecContentIssue | null {
	if (!body.includes(STUB_EXPLAINS_MARKER)) return null;
	const role = articleRoleFromPath(rel);
	const thresholds = (role ? ARTICLE_ROLE_THRESHOLDS[role] : undefined) ?? {
		minH2: 3,
		minBodyLines: 20,
	};
...
function checkScaffoldBoilerplate(
	rel: string,
	body: string,
): PlatformSpecContentIssue | null {
...
function checkStaleLegacyBridge(
	...
	for (const prefix of ["/execution/", "/corelib/"] as const) {
		...
		if (relatedTopicIsNonNormativeBridge(frontmatter.relatedTopics, prefix))
			continue;
...
function checkDuplicateManifestTables(
	websiteRoot: string,
	issues: PlatformSpecContentIssue[],
): void {
	const specRoot = path.join(
		websiteRoot,
		"src",
	...
	for (const pair of MANIFEST_FEATURE_PAIRS) {
		...
		for (const [dir, bucket] of [
			...
			for (const file of walk(dir)) {
				...
				const sigs = extractMarkdownTableSignatures(
					bodyAfterFrontmatter(fs.readFileSync(file, "utf8")),
				);
...
export function runPlatformSpecContentVerify(argv: string[]): void {
	...
	console.log(
		`platform-spec content verify [${modeLabel}]: scanned ${fileCount} file(s).`,
	);
	if (changedOnly)
		console.log("  filter: changed platform-spec paths only (--changed-only).");
	if (!warnOnly)
		console.log(
			"  allowlist: PSC001 skips 16 language-meta Tier-1 stub paths until Track A.",
		);
