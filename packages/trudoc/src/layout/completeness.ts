...
export function evaluateCompleteness(
	...
	for (const row of nodes) {
		...
		for (const rule of eff.effectiveSections) {
			let found = false;
			if (rule.kind === "specSection")
				found = satisfiesSpecSectionRule(signals, rule.id);
		...
		if (
			eff.effectiveMinSpecSections != null &&
			specCount < eff.effectiveMinSpecSections
		) {
			const m = diag(
				"MIN_SPEC_SECTIONS",
				"error",
		...
		if (
			eff.effectiveMinMarkdownHeadings != null &&
			h2Count < eff.effectiveMinMarkdownHeadings
		) {
			const m = diag(
				"MIN_MARKDOWN_H2",
				"error",
		...
		if (options?.docsRoot) {
			const absContent = path.join(
				options.docsRoot,
				...row.contentPath.split("/"),
			);
			...
			for (const m of validateHubMdxComponents(
				row.slug,
				row.contentPath,
				row.body,
			)) {
