...
export type RelatedTopicSeverity =
	| "informational"
	| "low"
	| "medium"
	| "high"
...
export function renderRelatedTopicsSection(
	...
	const rootClass = ["related-topics", options.rootClass]
		.filter(Boolean)
		.join(" ");

	if (!topics.length) {
		const msg = escapeHtml(
			options.emptyMessage ?? "No related spec documents declared.",
		);
	...
	const items = topics.map((t) => {
		const itemClass = [
			...
			t.blocker ? "related-topics__item--blocker" : "",
		]
			.filter(Boolean)
			.join(" ");
		...
		const blocker = t.blocker
			? `<span class="related-topics__blocker">Blocker</span>`
			: "";
