...
export function DownloadsSection({ initialVersion, versionSource }: DownloadsSectionProps) {
  const { version, assets, packages, installScript, loading, error } = useLatestVersion();
  const { releases, loading: releasesLoading } = useReleases();

  const [activePlatform, setActivePlatform] = useState<PlatformId>('linux-amd64');
  ...
  const activeAsset = useMemo<AssetInfo | undefined>(
    () => {
      ...
      return assets.find(
        (a) => a.platform === activePlatform || `${a.platform}-${a.arch}` === activePlatform,
      );
    },
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {displaySource && (
          <span className="text-xs text-muted-foreground">source: {displaySource}</span>
        )}
