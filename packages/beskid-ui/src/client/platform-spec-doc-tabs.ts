...
function handleSpecReaderLinkClick(event: MouseEvent) {
	...
	const link = target.closest<HTMLAnchorElement>(
		`${ARTICLE_LINK_SELECTOR}, ${TILE_LINK_SELECTOR}`,
	);
	if (
		!link ||
		link.closest('[role="tablist"]') ||
		!isLinkInVisibleTabPanel(link)
	)
		return;
	...
	for (const layout of root.querySelectorAll<HTMLElement>(
		...
		if (typeof localStorage !== "undefined") {
			localStorage.setItem(
				`starlight-synced-tabs__${SYNC_KEY}`,
				CURRENT_DOCUMENT_TAB,
			);
		}

		const tabAnchors = [
			...tabs.querySelectorAll<HTMLAnchorElement>('[role="tab"]'),
		];
		const index = tabAnchors.findIndex(
			(tab) => tab.textContent?.trim() === CURRENT_DOCUMENT_TAB,
		);
		const activeIndex = index >= 0 ? index : 0;
		const panels = [
			...tabs.querySelectorAll<HTMLElement>(':scope > [role="tabpanel"]'),
		];
...
function resetSpecReaderPanelScroll(layout: HTMLElement) {
	const main = layout.querySelector<HTMLElement>(
		".platform-spec-doc-split__main",
	);
	if (!main) return;
...
function scrollSpecReaderTabsIntoView(layout: HTMLElement) {
	layout
		.querySelector(".platform-spec-doc-tabs")
