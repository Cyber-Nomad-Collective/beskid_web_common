import {
	escapeHtml,
	type RelatedTopicPayload,
	renderRelatedTopicsSection,
} from "@cyber-nomad-collective/trudoc/platform-spec/related-topics-html";
...
const MAP_SEARCH_LEVEL_ORDER: GraphPayloadNode["level"][] = [
	"root",
	"domain",
	"area",
	"feature",
...
const GRAPH_LEVEL_CSS_VARS: Record<
	GraphPayloadNode["level"],
	{ fill: string; stroke: string; text: string }
> = {
	root: {
...
function linkPadding(
	source: GraphPayloadNode,
	target: GraphPayloadNode,
): number {
...
function linkDistanceBetween(
	source: GraphPayloadNode,
	target: GraphPayloadNode,
): number {
	return (
		effectiveNodeRadius(source) +
		effectiveNodeRadius(target) +
		linkPadding(source, target)
	);
}
...
function seedVisibleLayout(
	...
	for (const [parentId, group] of d3.group(
		areas,
		(n) => parents.get(n.id) ?? "",
	)) {
	...
	for (const [parentId, group] of d3.group(
		...
		const spread = Math.min(
			Math.PI * 1.35,
			Math.max(Math.PI / 3, (2 * Math.PI) / Math.max(6, group.length)),
		);
...
function syncMapChromeInsets() {
	...
	document.documentElement.style.setProperty(
		"--platform-spec-panel-bottom",
		`${bottomPx}px`,
	);
	mapPage.style.setProperty(
		"--platform-spec-available-height",
		`${available}px`,
	);
}
...
export function mountPlatformSpecGraph(): void {
	...
	const layoutEl = document.querySelector<HTMLElement>(
		"[data-platform-spec-graph-layout]",
	);
	...
	const mapPageEl = document.querySelector<HTMLElement>(
		".platform-spec-map-page",
	);
	...
	const applyMapLegendSwatches = () => {
		...
		for (const level of Object.keys(
			GRAPH_LEVEL_CSS_VARS,
		) as GraphPayloadNode["level"][]) {
			const el = legend.querySelector<HTMLElement>(
				`[data-platform-spec-legend-swatch="${level}"]`,
			);
			if (!el) continue;
	...
	const visibility = new Map(graph.nodes.map((n) => [n.id, !n.hidden]));
	for (const edge of graph.edges) {
	...
	const graphLayer = svg
		.append("g")
		.attr("class", "platform-spec-map-svg__layer");
	...
	function edgeVisible(edge: SimEdge): boolean {
		return Boolean(
			visibility.get(edgeEndpointId(edge.source)) &&
				visibility.get(edgeEndpointId(edge.target)),
		);
	}
	...
	const simulation = d3
		...
		.force(
			"collide",
			d3.forceCollide<SimNode>((n) => effectiveNodeRadius(n) + 14).iterations(2),
		);
	...
	function refreshCanvasSize(): boolean {
		...
		(simulation.force("center") as d3.ForceCenter<SimNode>)
			.x(width / 2)
			.y(height / 2);
		return true;
	}
	...
	function isHubAndDomainsOnly(active: SimNode[]): boolean {
		return (
			active.length > 0 &&
			active.every((n) => n.level === "root" || n.level === "domain")
		);
	}
	...
	function fitVisible(duration = 320) {
		...
		const scale = Math.max(
			0.35,
			Math.min(2.85, Math.min(width / boxW, height / boxH)),
		);
	...
	function collapseAllBelowDomains() {
		for (const node of graph.nodes) {
			if (node.level === "area" || node.level === "feature")
	...
	function toggleAreasForDomain(domain: string) {
		const areaNodes = graph.nodes.filter(
			(n) => n.level === "area" && n.domain === domain,
		);
		...
		for (const area of areaNodes) {
			...
			if (!nowHidden) {
				for (const feat of graph.nodes.filter(
					(n) => n.level === "feature" && n.areaPath === area.areaPath,
				)) {
	...
	function toggleFeaturesForArea(areaPath: string) {
		const features = graph.nodes.filter(
			(n) => n.level === "feature" && n.areaPath === areaPath,
		);
	...
	const relsEl = document.getElementById(
		"platform-spec-graph-panel-relations-rels",
	);
	const relCountEl = document.getElementById(
		"platform-spec-graph-panel-relations-count",
	);
	const relatedRoot = document.getElementById(
		"platform-spec-graph-panel-related-root",
	);
	const hintEl = document.getElementById("platform-spec-graph-panel-hint");
	const linkEl = document.getElementById(
		"platform-spec-graph-panel-link",
	...
	function focusNode(node: GraphPayloadNode) {
		...
		if (target) {
			...
			const scale = Math.min(
				zoomMax,
				Math.max(zoomMin + 0.02, baseScale * radiusBoost),
			);
			svg
				.transition()
				.duration(380)
	...
	function updatePanel(node: GraphPayloadNode) {
		if (!titleEl || !metaEl || !descEl || !dlEl || !relsEl || !hintEl || !linkEl)
			return;
		...
		const rows: string[] = [];
		if (node.ownerName)
			rows.push(`<dt>Owner</dt><dd>${escapeHtml(node.ownerName)}</dd>`);
		if (node.status)
			rows.push(`<dt>Spec standing</dt><dd>${escapeHtml(node.status)}</dd>`);
		if (node.specLevel)
			rows.push(`<dt>Spec level</dt><dd>${escapeHtml(node.specLevel)}</dd>`);
		...
		const outgoing = (outgoingByNode.get(node.id) ?? []).filter((e) =>
			visibility.get(e.to),
		);
		const incoming = (incomingByNode.get(node.id) ?? []).filter((e) =>
			visibility.get(e.from),
		);
		for (const e of outgoing) {
			const t = nodeById.get(e.to);
			if (!t) continue;
			relRows.push(
				`<li><strong>→ ${graphNodeLinkLabel(t)}</strong>${e.label ? ` (${escapeHtml(e.label)})` : ""}</li>`,
			);
		}
		...
		relsEl.innerHTML = relRows.length
			? relRows.join("")
			: "<li>No node relation notes available for visible edges.</li>";
	...
	const optsResetViewBtn = document.getElementById(
		"platform-spec-map-reset-view",
	);
	...
	const searchInput = document.getElementById(
		"platform-spec-map-search-input",
	) as HTMLInputElement | null;
	const searchResults = document.getElementById(
		"platform-spec-map-search-results",
	);
	...
	function renderSearchResults(term: string) {
		...
		const filtered = graph.nodes.filter((n) => {
			if (!q) return true;
			const hay =
				`${n.displayTitle ?? n.label} ${n.id} ${n.level} ${n.domain ?? ""} ${n.areaPath ?? ""}`.toLowerCase();
		...
		for (const level of MAP_SEARCH_LEVEL_ORDER) {
			const group = (byLevel.get(level) ?? []).sort((a, b) =>
				(a.displayTitle ?? a.label).localeCompare(
					b.displayTitle ?? b.label,
					undefined,
					{ sensitivity: "base" },
				),
			);
			...
			const items = group
				...
				.join("");
			sections.push(
				`<section class="platform-spec-map-search__group"><h4 class="platform-spec-map-search__group-title">${escapeHtml(MAP_SEARCH_LEVEL_HEADING[level])}</h4><ul class="platform-spec-map-search__group-list">${items}</ul></section>`,
			);
		}
		searchResults.innerHTML = sections.length
			? sections.join("")
			: '<p class="platform-spec-map-search__empty">No nodes match your search.</p>';
		if (searchClose && searchInput)
			searchClose.hidden = searchInput.value.trim().length === 0;
	}
	...
	if (urlNode) {
		const direct =
			nodeById.get(urlNode) ?? nodeById.get(nodeIdFromHref(urlNode) ?? "");
	...
	if (mapInitiallyVisible) {
		...
		cleanups.push(() =>
			window.removeEventListener("platform-spec-map-activate", activateMapLayout),
		);
	}
