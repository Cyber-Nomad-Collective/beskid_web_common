...
function initDownloadsPage(root: HTMLElement): void {
	...
	const directBtn = root.querySelector<HTMLAnchorElement>(
		"[data-direct-download]",
	);
	const directLabel = root.querySelector<HTMLElement>(
		"[data-direct-download-label]",
	);
	...
	if (detected) {
		...
		if (hintEl) {
			...
			if (/Intel Mac OS X|MacIntel/i.test(ua)) {
				...
				hintEl.hidden = false;
				hintEl.textContent =
					"Could not detect your OS — choose a platform tab above.";
			}
		}
	}
	...
	const vscodeCopyBtn =
		root.querySelector<HTMLButtonElement>("[data-copy-vscode]");
	const vscodeCopyLabel = vscodeCopyBtn?.querySelector<HTMLElement>(
		"[data-copy-vscode-label]",
	);
	const vscodeCmd =
		vscodeCopyBtn?.closest(".downloads-command")?.querySelector("code")
			?.textContent ?? "";
...
function init(): void {
	document
		.querySelectorAll<HTMLElement>("[data-downloads-page]")
		.forEach(initDownloadsPage);
}
