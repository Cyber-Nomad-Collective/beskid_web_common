...
function initPlatformSpecHomeTabs() {
	...
	if (!location.hash && history.replaceState) {
		history.replaceState(
			null,
			"",
			`${location.pathname}${location.search}#browse`,
		);
	}

	root
		.querySelectorAll<HTMLButtonElement>(".platform-spec-home__tab")
		.forEach((btn) => {
			btn.addEventListener("click", () =>
				selectTab(btn.dataset.tab === "browse" ? "browse" : "map"),
			);
		});
	...
	root.addEventListener("keydown", (e) => {
		...
		const active = root.querySelector<HTMLButtonElement>(
			'.platform-spec-home__tab[aria-selected="true"]',
		);
		if (!active) return;
