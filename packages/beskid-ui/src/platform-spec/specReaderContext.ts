...
import { type AdrSectionKey, parseAdrSections } from "./parseAdrSections";
...
function resolveArticleRootSlug(
	currentSlug: string,
...
export async function computeSpecReaderState(
	pathname: string,
	docs: DocsEntry[],
	cwd: string,
): Promise<SpecReaderComputed> {
	const path = normalizedPathname(pathname);
	const currentDoc = docs.find((entry) => docEntrySlug(entry) === path);
	const relatedTopics: RelatedTopicPayload[] = Array.isArray(
		currentDoc?.data?.relatedTopics,
	)
		? (currentDoc?.data?.relatedTopics as RelatedTopicPayload[])
		: [];
	const currentSpecLevel =
		typeof currentDoc?.data?.specLevel === "string"
			? currentDoc.data.specLevel
			: undefined;
		...
		currentDoc && articleRootSlug
			? docs
					.filter((entry) => {
						...
						if ((entry.data as { specLevel?: string }).specLevel !== "article")
							return false;
					...
					.map((entry) => ({
						title: String(
							entry.data.title ?? docEntrySlug(entry).split("/").at(-1) ?? "Untitled",
						),
						description:
							...
							entry.data.description.trim().length
								...
								: undefined,
						status:
							typeof entry.data.status === "string" ? entry.data.status : undefined,
						lastReviewed:
							typeof entry.data.lastReviewed === "string" ||
							entry.data.lastReviewed instanceof Date
								? String(entry.data.lastReviewed).slice(0, 10)
								: undefined,
						kind:
							typeof entry.data.specLevel === "string"
								? entry.data.specLevel
								: "article",
	...
	const adrCandidates =
		currentDoc && articleRootSlug
			? docs
					.filter((entry) => {
						const slug = docEntrySlug(entry);
						if ((entry.data as { specLevel?: string }).specLevel !== "adr")
							return false;
						return slug.startsWith(`${articleRootSlug}/adr/`);
					})
					.sort((a, b) => {
						const aId = String(
							(a.data as { adrId?: string }).adrId ?? docEntrySlug(a),
						);
						const bId = String(
							(b.data as { adrId?: string }).adrId ?? docEntrySlug(b),
						);
	...
	for (const entry of adrCandidates) {
		...
		for (const [key, markdown] of Object.entries(parsedSections) as [
			AdrSectionKey,
			string,
		][]) {
	...
	let architectureGraph: unknown;
	const archMeta = currentDoc?.data?.architectureGraph as
		| { source?: string }
		| undefined;
	if (archMeta?.source) {
...
export async function loadArchitectureGraphForEntry(
	entry: DocsEntry | undefined,
	cwd: string,
): Promise<unknown> {
	if (!entry) return undefined;
	const archMeta = entry.data?.architectureGraph as
		| { source?: string }
		| undefined;
